...
// 8 connected
int row[] = { -1, 0, 0, 1 , 1 , -1 , -1 , 1};     //Actions to perform in Dijkstra. 8 in total
...
int rowSize = sizeof(row)/sizeof(row[0]); // Number of actions available (8)
...
namespace global_planner {
    ...
    {
        if( cell.x >= size_x || cell.y >= size_y || cell.x < 0 || cell.y < 0){  //Checking if the cell is valid.
            return false;
        }
        return true;
    }
    ...
    bool GlobalPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,  std::vector<geometry_msgs::PoseStamped>& plan ){
        ...
        uint    mx_i, my_i, mx_f, my_f;         //costmap cell coordinates (strictly positive) in map  i stands for initial and f stands for final
        ...
        src.x = mx_i;
        src.y = my_i;
        src.dist = 0;       //Initial coordinates and distance
        
        pq.push(src);
        
        std::map<Cell, bool> visited;
        visited[src] = true;            //Map object to store visited nodes    
        
        std::map<Cell , Cell> parent;   // Map object to store parent nodes. Initial parent node set.
        parent[src] = src;
        ...
        while(!pq.empty()){
            ...
            uint x = curr_cell.x;       // storing coordinates of current cell
            uint y = curr_cell.y;

            //curr_cell distance will be already filled, beacuse we are setting it before push into the queue
            
            visited[curr_cell] = true;      //Storing current cell in visited 
            dist[curr_cell] = curr_cell.dist;  // Stroing current cell distance . For initial node it will be 0.
            ...
            for(int i=0 ; i < rowSize ; i++){
                ...
                Cell neigh;   // Cell for neighboring node
                neigh.x = n_x;  // neighboring cell coordinates.
                neigh.y = n_y; 
                //neigh.dist = set later

                int neigh_cost = (int)costmap_ros_->getCost(n_x, n_y); 
                if( neigh_cost > 200){continue;}                        // Checking for obstacles ( Higher cost returned from costmap, the position is occupied by obstacle (max possible value is 255))
                ...
                if(isValid(neigh)){
                    if(visited.find(neigh) == visited.end()){                            //i.e new cell found 
                        ...
                        if(dist.find(neigh) == dist.end()){ 
                            dist[neigh] = 10000;               // Setting distances to infinity initially.
                            neigh.dist  = 10000;
                        }
                        ...
                        if(curr_cell.dist + neigh_cost  < dist[neigh] ){  
                            ...
                            neigh.dist  = curr_cell.dist + neigh_cost;    // Updating distance of the neighboring cell from current cell using the cost. 
                            dist[neigh] = curr_cell.dist + neigh_cost;    // Heuristic here is the cost to reach the new cell.
                            visited[neigh] = true;
            ...
            //if goal found
                if(x == mx_f && y == my_f){
                    ...
                    while(true){
                        ...
                        if(last_cell.x == src.x && last_cell.y == src.y){ break;}       // Backtracking
                    ...
                    for(int i=0 ; i < my_path.size(); i++){
                        ...
                        costmap_ros_->mapToWorld(mx, my, wx, wy);           //Converting to world coordinates to give back to the planner.

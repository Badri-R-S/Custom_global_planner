...
// 8 connected
int row[] = { -1, 0, 0, 1 , 1 , -1 , -1 , 1};   //actions
...
namespace global_planner {
    ...
    void GlobalPlanner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros){
        ...
        size_x = costmap_ros_->getSizeInCellsX(); //max size of costmap.
    ...
    bool GlobalPlanner::isValid(Cell cell)
    {
        if( cell.x >= size_x || cell.y >= size_y || cell.x < 0 || cell.y < 0){ 
            return false;                                                           //Checking if a cell is valid or not.
        }
        return true;
    }
    ...
    bool GlobalPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,  std::vector<geometry_msgs::PoseStamped>& plan ){
        ...
        Cell src;
        src.x = mx_i;               //Setting initial map coordinates
        src.y = my_i;

        q.push(src);                //Pushing the initial position to queue
        
        std::map<Cell, bool> visited;  //Map object to store if a given cell is visited or not.
        visited[src] = true;             //setting initial cell as visited

        std::map<Cell , Cell> parent;   //Map object to set parent.
        parent[src] = src;
        ...
        while(!q.empty()){
            ...
            for(int i=0 ; i < rowSize ; i++){
                ...
                if((int)costmap_ros_->getCost(n_x, n_y) > 200 ) {continue;} //Having obstacle at the location will have a huge cost in the map, Hence, if obstacle found, skip loop.
